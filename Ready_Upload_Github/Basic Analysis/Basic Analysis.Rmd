...
# News sentiment score for Exxon (factor we want to investigate)
```{r echo=FALSE}
news_sentiment <- read.csv("Basic_analysis_data/ave-daily-news-sentiment-exxon-2020.csv")
...
# NYSE stock closing price for Exxon (target variable)
```{r echo=FALSE, message=FALSE}
NYSE_stock <- 
...
Exxon_stock <- NYSE_stock[which(NYSE_stock$Stock.Name == "EXXON MOBIL CORPORATION"),]
```

## Basic summary of Exxon stock price
```{r echo=FALSE}
par(mfrow=c(1,2))
hist(Exxon_stock$`Stock Price`, main = '', xlab = 'Exxon Stock Price')
plot(Exxon_stock$`Stock Price` ~ as.Date(Exxon_stock$Date), xlab = 'Date', ylab = 'Exxon stock price')
```
...
One should clearly observe that the trend of crude oil commodity and Exxon stock price are consistent.  
```{r echo=FALSE}
par(mfrow=c(1,3))
plot(Exxon_stock$`Stock Price` ~ as.Date(Exxon_stock$Date), xlab = 'Date', ylab = 'Exxon stock price',
     main = 'Exxon')
...
```{r echo=FALSE, message=FALSE}
exxon_p <- ggplot(Exxon_stock, aes(x = Date, y = `Stock Price`)) + geom_line()+ labs(title = 'Exxon')
exxon_hist <- ggplot(Exxon_stock, aes(x = `Stock Price`)) + geom_histogram()+labs(title = 'Exxon')

ice_brent_p <- ggplot(ice_brent, aes(x = Date, y = Price)) + geom_line() + labs(title = 'Ice Brent Settle')
ice_brent_hist <- ggplot(ice_brent, aes(x = Price)) + geom_histogram() + labs(title = "Ice Brent Settle")

NYMEX_p <- ggplot(NYMEX, aes(x = Date, y = Price)) + geom_line() + labs(title = 'NYMEX Settle')
NYMEX_hist <- ggplot(NYMEX, aes(x = Price)) + geom_histogram() + labs(title = 'NYMEX Settle')

grid.arrange(exxon_p, ice_brent_p, NYMEX_p, nrow = 1, ncol = 3)
```
...
```{r echo=FALSE}
common_date <- Reduce(intersect, list(news_sentiment$date, Exxon_stock$Date, ice_brent$Date,
...
Exxon_stock_price <- Exxon_stock[Exxon_stock$Date %in% common_date,]$`Stock Price`
...
df <- data.frame(date, Exxon_stock_price, news_sentiment_score, ice_brent_settle, NYMEX_settle, DJ_comp,
...
We noticed that the news sentiment is somewhat correlated with Exxon Stock price, Dow Jones Utility average and apple mobility transit index.  
...
```{r echo=FALSE}
#write.csv(df,"C:/Users/guanq/OneDrive/News_data/organized_exxon.csv", row.names = FALSE)
```
